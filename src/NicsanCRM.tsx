...
function PageUpload() {
  ...
  const handleFiles = async (files: FileList) => {
    ...
    try {
      ...
      if (result.ok || result.success) {
        ...
        if (uploadData) {
        const newFile = {
            id: uploadData.uploadId || uploadData.id,
          ...
          setUploadedFiles(prev => {
            console.log("🔍 Adding new file with ID:", newFile.id);
            console.log("🔍 Existing files:", prev.map(f => f.id));
            return [newFile, ...prev];
          });
          ...
          pollUploadStatus(uploadData.uploadId || uploadData.id);
        }
  ...
  const pollUploadStatus = async (uploadId: string) => {
    if (!uploadId || uploadId === 'undefined') {
      console.warn('pollUploadStatus called with invalid uploadId:', uploadId);
      return;
    }
              ...
              <tbody>
                {uploadedFiles.length === 0 ? (
                  ...
                  uploadedFiles.map((file, index) => {
                    console.log(`🔍 Rendering file ${index}:`, { id: file.id, filename: file.filename });
                    return (
                    <tr key={file.id || `file-${index}`} className="border-t">
...
function PageReview({ user }: { user: {name:string; email?:string; role:"ops"|"founder"} }) {
  ...
  // Phase 0: Stable form state - seeded from API once, never re-seeded unless uploadId changes
  ...
  // Phase 0: Map API data to flat form structure for controlled inputs
  const mapApiToForm = (d: any) => ({
    policy_number: d.policy_number?.value ?? '',
    vehicle_number: d.vehicle_number?.value ?? '',
    issue_date: d.issue_date?.value ?? '',
    expiry_date: d.expiry_date?.value ?? '',
    idv: d.idv?.value ?? '',
    total_premium: d.total_premium?.value ?? '',
    make: d.make?.value ?? '',
    model: d.model?.value ?? '',
    variant: d.variant?.value ?? '',
    fuel_type: d.fuel_type?.value ?? '',
    insurer: d.insurer?.value ?? '',
    product_type: d.product_type?.value ?? 'MOTOR',
    cc: d.cc?.value ?? '',
    manufacturing_year: d.manufacturing_year?.value ?? '',
    ncb: d.ncb?.value ?? '',
    discount: d.discount?.value ?? '',
    net_od: d.net_od?.value ?? '',
    total_od: d.total_od?.value ?? '',
    net_premium: d.net_premium?.value ?? ''
  });
  ...
  // Phase 0: Seed form data when uploadId changes (never re-seed unless uploadId changes)
  React.useEffect(() => {
    if (selectedUploadId && selectedUploadId !== lastUploadId && reviewData?.extracted_data) {
      console.log('🔄 Seeding form data for new uploadId:', selectedUploadId);
      console.log('🔄 reviewData:', reviewData);
      console.log('🔄 reviewData.extracted_data:', reviewData.extracted_data);
      const extracted = reviewData.extracted_data;
      const seededForm = mapApiToForm(extracted);
      setFormData(seededForm);
      setLastUploadId(selectedUploadId);
      console.log('✅ Form data seeded:', seededForm);
    }
  }, [selectedUploadId, reviewData, lastUploadId]);
  ...
  const handleExtractWithModel = async (model: 'primary' | 'secondary') => {
    ...
    try {
      ...
      if ((result.success || result.ok) && result.data) {
        ...
        console.log("🔎 Extraction API raw:", result);
        console.log("📊 Result.data structure:", result.data);
        console.log("📋 Sample field (policy_number):", result.data.policy_number);
        
        // Ensure data is properly structured for the field() function
        const normalized = { extracted_data: result.data };
        console.log("✅ Normalized state:", normalized);
        console.log("📊 Field count:", Object.keys(result.data).length);
        ...
        setSubmitMessage({ 
          type: 'success', 
          message: `✅ Extraction successful with ${model} model - Found ${Object.keys(result.data).length} fields. You can now review the data below or go to Review & Confirm page.` 
        });
        
        // Update upload status to 'review' after successful extraction
        setUploads(prev => prev.map(upload => 
          upload.id === selectedUploadId 
            ? { ...upload, status: 'review', upload_status: 'review' }
            : upload
        ));
      } else {
  ...
  const handleExtractOCR = async (uploadId: string) => {
    ...
    try {
      ...
      if ((result.success || result.ok) && result.data) {
        // Debug: Log the raw API response
        console.log("🔎 OCR Extraction API raw:", result);
        console.log("📊 OCR Result.data structure:", result.data);
        console.log("📋 OCR Sample field (policy_number):", result.data.policy_number);
        
        // Ensure data is properly structured for the field() function
        const normalized = { extracted_data: result.data };
        console.log("✅ OCR Normalized state:", normalized);
        console.log("📊 OCR Field count:", Object.keys(result.data).length);
        ...
        setSubmitMessage({ 
          type: 'success', 
          message: `✅ OCR extraction successful - Found ${Object.keys(result.data).length} fields. You can now review the data below or go to Review & Confirm page.` 
        });
        
        // Update upload status to 'review' after successful OCR extraction
        setUploads(prev => prev.map(upload => 
          upload.id === uploadId 
            ? { ...upload, status: 'review', upload_status: 'review' }
            : upload
  ...
  const handleLoadUploadData = async () => {
    ...
    console.log("📂 Loading upload data for selectedKey:", selectedKey);
  ...
  console.log("🔍 Current reviewData:", reviewData);
  console.log("🔍 Current extracted_data:", safeReview.extracted_data);
  console.log("🔍 Current extracted variable:", extracted);
  console.log("🔍 Sample field access test - policy_number:", extracted?.policy_number);
  console.log("🔍 Current formData:", formData);
  console.log("🔍 Current selectedUploadId:", selectedUploadId);

  // Phase 0: UI Helper to fix [object Object] rendering
  const f = (x?: {value?: any, source?: string, confidence?: number}) => ({
    value: x?.value ?? '',
  ...
  const field = (k: string, fallback: any = '') => {
    const extractedValue = f(extracted?.[k]).value;
    const safePdfValue = safePdfData?.[k];
    // Handle case where safePdfData[k] might be an object with a value property
    const safePdfValueExtracted = (typeof safePdfValue === 'object' && safePdfValue?.value !== undefined) 
      ? safePdfValue.value 
      : safePdfValue;
    const result = extractedValue || safePdfValueExtracted || fallback;
    console.log(`🔑 field() lookup: ${k} → ${result} (from extracted: ${extracted?.[k]?.value}, from safePdfData: ${safePdfValue})`);
    return result;
  };
      ...
      <Card title="Review & Confirm" desc="Review PDF data + manual extras before saving">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <LabeledInput 
              label="Policy Number" 
              value={formData.policy_number || f(extracted.policy_number).value}
              onChange={(e) => setFormData((prev: any) => ({...prev, policy_number: e.target.value}))}
              hint="auto-read from PDF"
            />

            <LabeledInput 
              label="Vehicle Number" 
              value={formData.vehicle_number || f(extracted.vehicle_number).value}
              onChange={(e) => setFormData((prev: any) => ({...prev, vehicle_number: e.target.value}))}
              hint="check format"
            />
            <LabeledInput 
              label="Insurer" 
              value={formData.insurer || f(extracted.insurer).value}
              onChange={(e) => setFormData((prev: any) => ({...prev, insurer: e.target.value}))}
            />
            <LabeledInput 
              label="Product Type" 
              value={f(extracted.product_type).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="Make" 
              value={formData.make || f(extracted.make).value}
              onChange={(e) => setFormData((prev: any) => ({...prev, make: e.target.value}))}
            />
            <LabeledInput 
              label="Model" 
              value={formData.model || f(extracted.model).value}
              onChange={(e) => setFormData((prev: any) => ({...prev, model: e.target.value}))}
            />
            <LabeledInput 
              label="CC" 
              value={f(extracted.cc).value}
              onChange={() => {}} // Read-only for review
              hint="engine size"
            />
            <LabeledInput 
              label="Manufacturing Year" 
              value={f(extracted.manufacturing_year).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="Issue Date" 
              value={f(extracted.issue_date).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="Expiry Date" 
              value={f(extracted.expiry_date).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="IDV (₹)" 
              value={f(extracted.idv).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="NCB (%)" 
              value={f(extracted.ncb).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="Discount (%)" 
              value={f(extracted.discount).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="Net OD (₹)" 
              value={f(extracted.net_od).value}
              onChange={() => {}} // Read-only for review
              hint="Own Damage"
            />
            <LabeledInput 
              label="Total OD (₹)" 
              value={f(extracted.total_od).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="Net Premium (₹)" 
              value={f(extracted.net_premium).value}
              onChange={() => {}} // Read-only for review
            />
            <LabeledInput 
              label="Total Premium (₹)" 
              value={formData.total_premium || f(extracted.total_premium).value}
              onChange={(e) => setFormData((prev: any) => ({...prev, total_premium: e.target.value}))}
            />
          </div>
        </div>
...
export default function NicsanCRM() {
